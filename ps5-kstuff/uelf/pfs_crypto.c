...
int pfs_xts_virtual(uint64_t dst, uint64_t src, const uint8_t* key, uint64_t start, uint32_t count, int is_encrypt)
{
    ...
    if(isal_aes_keyexp_128(key + 16, data_key_enc, data_key_dec))
        goto exit;
    if(isal_aes_keyexp_128(key, tweak_key_enc, tweak_key_dec))
        goto exit;
    while(count)
    {
        ...
        && src_end - src_phys >= SECTOR_SIZE && dst_end - dst_phys >= SECTOR_SIZE)
        {
            ...
            while(run--)
            {
                ...
                int err = is_encrypt
                    ? isal_aes_xts_enc_128_expanded_key(tweak_key_enc, data_key_enc, (void*)tweak,
                                                        SECTOR_SIZE, DMEM + src_phys, DMEM + dst_phys)
                    : isal_aes_xts_dec_128_expanded_key(tweak_key_enc, data_key_dec, (void*)tweak,
                                                        SECTOR_SIZE, DMEM + src_phys, DMEM + dst_phys);
                if(err)
                    goto exit;
        ...
        if(copy_from_kernel(input, src, SECTOR_SIZE))
            goto exit;
        if(is_encrypt) {
            if(isal_aes_xts_enc_128_expanded_key(tweak_key_enc, data_key_enc, (void*)tweak,
                                                 SECTOR_SIZE, input, output))
                goto exit;
        } else {
            if(isal_aes_xts_dec_128_expanded_key(tweak_key_enc, data_key_dec, (void*)tweak,
                                                 SECTOR_SIZE, input, output))
                goto exit;
        }
